...
function generateCSV(delimiter = ',') {
  ...
  const rows = crops.map(crop => {
    ...
    return [
      ...
      nutritionStr,
      productsStr
    ].map(val => escapeCSV(val)).join(delimiter);
  });

  return '\uFEFF' + [headers.map(h => escapeCSV(h)).join(delimiter), ...rows].join('\n');
}

const rootDir = path.join(__dirname, '..');

// Write standard CSV (comma separated)
fs.writeFileSync(path.join(rootDir, 'cultivos_agrofert.csv'), generateCSV(','), 'utf8');

// Write Excel CSV (semicolon separated for Spanish locale Excel)
fs.writeFileSync(path.join(rootDir, 'cultivos_agrofert_excel.csv'), generateCSV(';'), 'utf8');
